...
def get_weights(file_name="bayesian_weights.txt"):
    with open(file_name, "rb") as MyFile:
        concepts_weight = pickle.load(MyFile)
        return concepts_weight


def save_weights(concepts_weights):
    with open("bayesian_weights.txt", "wb") as MyFile:
...
class bayesian:
    ...
    def calculate_bayesian(self):
        filesize = os.path.getsize("bayesian_weights.txt")
        if filesize == 0:
            concepts_weights = self.calculate_weights()
            save_weights(concepts_weights)
        else:
        ...
        while len(queue) != 0:
            ...
            if children_number == 0:
                ...
                for child in children:
                    queue.append((child, (weight / children_number)*weight))
